...
*
*   Why is this needed?
*   - In IDOPROFITIDOCOKE some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all IDOPROFITIDOCOKE specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f COKE\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
