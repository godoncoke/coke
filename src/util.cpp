...
//IDOPROFITIDOCOKE only features
...
const char * const BITCOIN_CONF_FILENAME = "idoprofitidocoke.conf";
const char * const BITCOIN_PID_FILENAME = "idoprofitidocoked.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "idoprofitidocoke" is a composite category enabling all IDOPROFITIDOCOKE-related debug output
            if(ptrCategory->count(string("idoprofitidocoke"))) {
...
#else
    const char* pszModule = "idoprofitidocoke";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IDOPROFITIDOCOKECore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IDOPROFITIDOCOKECore
    // Mac: ~/Library/Application Support/IDOPROFITIDOCOKECore
    // Unix: ~/.idoprofitidocokecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IDOPROFITIDOCOKECore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IDOPROFITIDOCOKECore";
#else
    // Unix
    return pathRet / ".idoprofitidocokecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty idoprofitidocoke.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override idoprofitidocoke.conf
