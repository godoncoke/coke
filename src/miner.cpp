...
// ***TODO*** ScanHash is not yet used in IDOPROFITIDOCOKE
//
...
{
    LogPrintf("IDOPROFITIDOCOKEMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("idoprofitidocoke-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("IDOPROFITIDOCOKEMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("IDOPROFITIDOCOKEMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("IDOPROFITIDOCOKEMiner:\n  proof-of-work found\n  hash: %s\n  target: %s\n", hash.GetHex(), hashTarget.GetHex());
    ...
    {
        LogPrintf("IDOPROFITIDOCOKEMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("IDOPROFITIDOCOKEMiner -- runtime error: %s\n", e.what());
        return;
    }
}
