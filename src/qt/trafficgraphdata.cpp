...
const int TrafficGraphData::IDOPROFITIDOCOKEdQueueSizes[] = {
...
void TrafficGraphData::tryAddingSampleToStash(GraphRange range)
{
    ...
    if (queue.size() > IDOPROFITIDOCOKEdQueueSizes[range]){
           sampleStash[range].push_front(queue.at(IDOPROFITIDOCOKEdQueueSizes[range]));
    }
}
...
{
    SampleQueue& queue = sampleMap[range];
    if (queue.size() == IDOPROFITIDOCOKEdQueueSizes[range] + 2){
